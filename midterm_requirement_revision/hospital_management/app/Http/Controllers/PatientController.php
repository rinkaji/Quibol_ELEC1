...
class PatientController extends Controller
{
    ...
    public function deletePatient($id)
    {
        ...
        DB::delete('delete from findings where patient_id =? ', [$id]);
    ...
    public function edit(Request $request, $id)
    {
        ...
        $sex = $request->input('patientSex');
        $addr = $request->input('patientAddress');
        $update = now()->setTimezone('Asia/Manila');
        DB::update('update patient set name = ?, age = ?, sex = ?, address = ?, updated_at = ?  where id = ?', [$name, $age, $sex, $addr, $update, $id]);
